...
Since #116 the summary also prints a **data-rights note** under the table: `discover` performs user-initiated acquisition from the pinned upstream (this project redistributes no dataset content), and each discovered dataset is listed with its upstream license and review state (e.g. `fleurs  license: CC-BY-4.0  redistribution review: pending`). Data whose review is not approved cannot pass the publish gate (`catalog.identity(publishable=True)`); see `docs/import-gate.md`, "Acquisition vs. redistribution".
...
**Expected:** exit 1. A single `ERROR ... Skipping model whisper_base_en — could not be constructed: StandardHuggingFaceASR (openai/whisper-base) hands audio file paths to the transformers ASR pipeline, which requires the ffmpeg binary to decode them — including WAV.` naming install commands (`apt-get install ffmpeg` / `brew install ffmpeg`) and the ffmpeg-free adapter alternatives, followed by `Evaluation failed: None of the requested models could be constructed` (since D40, one unconstructible model is skipped rather than aborting the run; with a single model the run then has nothing to evaluate). No per-utterance `Transcription failed` lines, no checkpoint download, no `scores_*.json` claiming success, no traceback. In a multi-model run, self-decoding adapters (`wav2vec2_*`) still evaluate.
...
| D44 | All 8 catalog benchmarks were `review.decision: pending` with no approver/evidence, 7 of them enabled and downloadable via `discover` — contradicting the import gate's "documented license **and** confirmed redistribution rights" bar, with no stated position on why pending entries are acquirable | **Documented / surfaced** | The position is now written down (`docs/import-gate.md`, "Acquisition vs. redistribution", and the catalog header): the catalog stores pointers, never content; `discover` is user-initiated acquisition from the pinned upstream under the per-entry license; `review.decision` gates project redistribution/publishing, which `catalog.identity(publishable=True)` already refuses for non-approved data. `discover` now prints a data-rights note listing each dataset's license and review state. `common_voice` is disabled for its retired upstream, not its review state; `prohibited` ⇒ disabled is now pinned by a test. **Closing the pending reviews themselves (approver, date, evidence URL, fingerprints) remains a maintainer/rights action, not a code change.** |
